...
hQuery.Medication = (function(_super) {
  ...
  Medication.prototype.typeOfMedication = function() {
    var _ref, _ref1;
    return new hQuery.TypeOfMedication((_ref = this.json['typeOfMedication']) != null ? _ref['code'] : void 0, (_ref1 = this.json['typeOfMedication']) != null ? _ref1['code_system'] : void 0);
  };
  ...
  Medication.prototype.statusOfMedication = function() {
    var _ref, _ref1;
    return new hQuery.StatusOfMedication((_ref = this.json['statusOfMedication']) != null ? _ref['code'] : void 0, (_ref1 = this.json['statusOfMedication']) != null ? _ref1['code_system'] : void 0);
  };
...
hQuery.Immunization = (function(_super) {
  ...
  Immunization.prototype.refusalReason = function() {
    var _ref, _ref1;
    return new hQuery.NoImmunization((_ref = this.json['negationReason']) != null ? _ref['code'] : void 0, (_ref1 = this.json['negationReason']) != null ? _ref1['code_system'] : void 0);
  };
...
hqmf.SpecificOccurrence = (function() {
  ...
  SpecificOccurrence.prototype.SUM = function(range) {
    return this.applyRangeSubset(SUM, range);
  };

  SpecificOccurrence.prototype.MEDIAN = function(range) {
    return this.applyRangeSubset(MEDIAN, range);
  };
...
Row = (function() {
  ...
  Row.prototype.toHashKey = function() {
    var string, _ref;
    return string = this.flattenToIds().join(",") + ("," + ((_ref = this.tempValue) != null ? _ref.id : void 0));
  };
...
var ANYNonNull, ActiveDays, CD, CMD, CONCURRENT, COUNT, CodeList, CrossProduct, DATEDIFF, DATETIMEDIFF, DURING, EACW, EACWS, EAE, EAS, EBCW, EBCWS, EBDU, EBE, EBS, ECW, ECWS, EDU, FIELD_METHOD_UNITS, FIFTH, FIRST, FOURTH, INTERSECT, IVL_PQ, IVL_TS, LAST, MAX, MEDIAN, MIN, OVERLAP, PQ, RECENT, SACW, SACWE, SAE, SAS, SBCW, SBCWE, SBDU, SBE, SBS, SCW, SCWE, SDU, SECOND, SUM, THIRD, TIMEDIFF, TS, UNION, XPRODUCT, adjustBoundsForField, allFalse, allTrue, anyMatchingValue, applySpecificOccurrenceSubset, atLeastOneFalse, atLeastOneTrue, boundAccessor, dateSortAscending, dateSortDescending, denormalizeEvent, denormalizeEventsByLocation, denormalizeEventsByTransfer, evalUnlessShortCircuit, eventAccessor, eventMatchesBounds, eventsMatchBounds, fieldOrContainerValue, filterEventsByField, filterEventsByValue, getCodes, getIVL, getTS, hqmfjs, invokeAll, invokeOne, matchingValue, narrowEventForFacility, selectConcurrent, shiftTimes, uniqueEvents, valueSortAscending, valueSortDescending, withinRange,
  __slice = [].slice,
...
IVL_PQ = (function() {
  ...
  IVL_PQ.prototype.normalizeToMins = function() {
    var _ref, _ref1;
    return new IVL_PQ((_ref = this.low_pq) != null ? _ref.normalizeToMins() : void 0, (_ref1 = this.high_pq) != null ? _ref1.normalizeToMins() : void 0);
  };
...
IVL_TS = (function() {
  ...
  IVL_TS.prototype.match = function(other) {
    ...
    other = getTS(other, ((_ref = this.low) != null ? _ref.inclusive : void 0) || ((_ref1 = this.high) != null ? _ref1.inclusive : void 0));
...
CrossProduct = (function(_super) {
  ...
  CrossProduct.prototype.intersect = function() {
    var currentIds, index, result, _i, _ref;
    result = this.childList(0) || [];
    for (index = _i = 1, _ref = this.listCount(); _i < _ref; index = _i += 1) {
...
INTERSECT = function() {
  var eventLists;
  eventLists = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
  return hqmf.SpecificsManager.intersectAll((new CrossProduct(eventLists)).intersect(), eventLists);
};
...
SUM = function(events, range, initialSpecificContext, fields) {
  var comparison, event, field, result, sum, unit, _i, _j, _len, _len1;
  sum = 0;
  comparison = range;
  ...
  return applySpecificOccurrenceSubset('SUM', hqmf.SpecificsManager.maintainSpecifics(result, events), range);
};
...
MEDIAN = function(events, range, initialSpecificContext, fields) {
    ...
    if (field) {
      ...
      if (field === 'values') {
        values = (function() {
          ...
          for (_i = 0, _len = events.length; _i < _len; _i++) {
            event = events[_i];
            _results.push(event[field]()['scalar']);
          }
          return _results;
        })();
      } else {
  ...
  return applySpecificOccurrenceSubset('MEDIAN', hqmf.SpecificsManager.maintainSpecifics(result, events), range);
};
...
ActiveDays = (function() {
  ...
  ActiveDays.prototype.add_days_from = function(start, number_of_days) {
    var diff, x, _i, _ref, _results;
    _results = [];
    for (x = _i = 0, _ref = number_of_days - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; x = 0 <= _ref ? ++_i : --_i) {
      diff = (1000 * 60 * 60 * 24) * x;
      _results.push(this.add_date(new Date(start.getTime() + diff)));
    }
    return _results;
  };
  ...
  ActiveDays.prototype.print_days_in_range = function(low, high) {
    var end, formated_end, formated_start, start, str, x, _i, _len, _ref;
    start = this.as_date(low);
    end = this.as_date(high);
    formated_start = this.format_date(start);
    formated_end = this.format_date(end);
    ...
    _ref = this.days_active(low, high);
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      x = _ref[_i];
...
CMD = (function(_super) {
  ...
  CMD.prototype.add_medication = function(medication) {
    var dose, dosesPerDay, fh, fills, history, oi, startDate, totalDays, _i, _j, _len, _len1, _ref, _results, _results1;
    dose = medication.dose().value();
    dosesPerDay = medication.administrationTiming().dosesPerDay();
    if (this.calculation_type === "order") {
      _ref = medication.orderInformation();
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        oi = _ref[_i];
        ...
        if (!isNaN(totalDays)) {
          ...
          fills = oi.fills() || 1;
          _results.push(this.add_days_from(startDate, totalDays * fills));
        } else {
      ...
      for (_j = 0, _len1 = history.length; _j < _len1; _j++) {
        ...
        if (!isNaN(totalDays)) {
          ...
          _results1.push(this.add_days_from(startDate, totalDays));
        } else {
...
hQuery.Medication.prototype.cumulativeMedicationDuration = function(dateRange) {
  if (this.administrationTiming() && this.dose()) {
